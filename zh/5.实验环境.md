...
- rsync：在有ssh登录能力下的文件/文件夹传输工具。python3 -m http.server 12771命令可以开启http服务，也可以用于文件传输（使用wget命令或者浏览器下载）。nc 也可以来传递文件，不过这种传输文件的手法在渗透测试的场景下更常见。
...
- tmux：terminal管理工具，可以切分命令行界面或者后台跑任务（防止ssh断开导致实验任务中断）
...
- docker代理：涉及到两个环节，镜像加速器和daemon代理
...
最后需要强调的是实验机器的安全性，最起码要避免弱密码，建议使用私钥登录方式，本地生成、保管好公私钥对，将公钥上传到实验机器上，可参考以下两个连接：
...
- GitHub学生包：里面包含了一整年的 GitHub Copilot Pro 会员
- 阿里云学生认证300优惠券：可用于抵扣阿里云百炼平台的 API 调用费用
- Gemini Pro 会员：商业版可免费使用一个月，美国大学生可申请一年免费会员
- Google Colab：白嫖显卡

## 实践任务

纸上得来终觉浅，以下是根据个人开发经验所设置的基础任务：

- 安装 VSCode 并配置多种常用插件，尝试使用本地私钥通过 VSCode 远程连接服务器
- 在远程服务器上安装 Docker 服务，解决网络问题，映射 Docker 容器中的 ssh 端口到服务器端，并使容器后台运行后，进入容器的 bash 环境，为其安装 SSH 服务
- 在容器中搭建实验环境，尝试在 vscode 中右键下载、拖拽上传等功能，并使用 python web 服务、nc 、rsync 等工具进行文件传输
- 安装命令行代理工具，使用 git 从 GitHub 中拉取仓库到容器中
